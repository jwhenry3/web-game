...
export interface PipoyaTilesetDef {
  ...
  image: string;
  primary?: boolean;
  /** Override public dir when the sheet lives outside PIPOYA_PUBLIC_BASE. */
  base?: string;
}
...
export const PIPOYA_TILESETS: PipoyaTilesetDef[] = [
  ...
  // Clara Mundi's custom supplemental set — generated by tools/gen_mundi_terrain.py.
  {
    id: "munditerrain",
    name: "MundiTerrain",
    firstgid: 6000,
    tilecount: 64,
    columns: 8,
    image: "MundiTerrain.png",
    base: "/assets/tilesets/mundi",
  },
];
...
export function loadPipoyaSheets(base = PIPOYA_PUBLIC_BASE): Promise<LoadedPipoyaSheet[]> {
  ...
  loadPromise = Promise.all(
    PIPOYA_TILESETS.map(async (def) => {
      const img = await loadImage(`${def.base ?? base}/${def.image}`);
      return {
        ...def,
        img,
        tileWidth: 32,
