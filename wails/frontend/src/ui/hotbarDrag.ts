...
export interface HotbarDrag {
  kind: "skill" | "item";
  id: string;
  /** Source slot when dragging a bound slot; absent for palette/inventory drags. */
  slot?: string;
}
...
export function writeHotbarDrag(e: DragEvent, payload: HotbarDrag) {
  ...
  // dataTransfer payloads are unreadable until drop, so the bar tracks the
  // in-flight drag via game state to highlight itself during dragover.
