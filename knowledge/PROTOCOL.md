...
*   **Traceability is Law:** Every claim generated must be traceable back to an immutable source in the `raw/` directory, OR via an executed Tool/API call that was logged and stored as a structured Source.
*   **System First:** Always check or update `./wiki/index.md` after any major write operation.
...
2.  **Summarize:** Create a structured summary page in `./wiki/sources/[source-name].md` containing key takeaways and high-level findings.
3.  **Identify Concepts:** Extract all major concepts (e.g., 'Attention', 'Embeddings') and ensure dedicated, up-to-date concept pages exist in `./wiki/concepts/`. If a page doesn't exist, create it with the standard YAML frontmatter template from `CLAUDE.md`.
...
5.  **Cataloging:** Update `./wiki/index.md` with new entries and append a detailed log entry to `./wiki/log.md`.

#### 3. Operational Workflow: Tool-Assisted Retrieval (Web Search Protocol)
When external, real-time data is needed (e.g., current events, live product details):
1.  **Tool Selection:** Identify the appropriate tool and API endpoint required (e.g., `web_search` using a dedicated search service).
2.  **Parameter Definition:** Define clear input parameters (the query, time constraints, etc.).
3.  **Execution & Logging:** Execute the tool call. **The entire API response and request details must be logged to `raw/tool_logs/` for full traceability.**
4.  **Data Extraction & Filtering:** Do not simply pass the raw output. Analyze the structured data (JSON) and extract only the *most relevant, verifiable* facts.
5.  **Integration:** Treat the extracted facts as if they were a newly discovered source document, proceeding to Step 2 of 'Ingest' above (Summary Creation, Concept Identification, etc.).
When answering a user query:
1.  **Index Consult:** First, read `./wiki/index.md` to determine the most relevant concepts or sources. Do not rely on brute-force searching all files unless necessary.
...
4.  **Knowledge Compound:** If the synthesized answer is novel and valuable enough to warrant permanent storage, proactively draft a new page in `./wiki/comparisons/` or `./wiki/concepts/` for user review.
...
*Self-Correction: Always reference `./wiki/CLAUDE.md` when unsure about conventions.*
