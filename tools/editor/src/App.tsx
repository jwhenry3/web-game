...
export function App() {
  ...
  const [char, setChar] = useState(
    () => new URLSearchParams(location.search).get("char") || "h99doll",
  );
  ...
  /** Resolve the attachment key shown for a slot under the current variant selection. */
  ...
  const switchChar = useCallback(
    (id: string) => {
      ...
      setAnimTime(0);
      setAdditiveBase("");
      history.replaceState(null, "", `?char=${encodeURIComponent(id)}`);
    },
    [char, dirty],
  );
  ...
  const doDelete = useCallback(async () => {
    ...
    try {
      ...
      await refreshChars();
      setChar("h99doll");
      history.replaceState(null, "", "?char=h99doll");
  ...
  return (
    <div className="ed-root">
      <header className="ed-titlebar">
        ...
        <strong>Spine Editor</strong><span className="ed-title-divider">/</span>
        <span>{char}{dirty ? " *" : ""}</span>
        <span className="spacer" /><span className="ed-hint">Character workspace</span>
      </header>
      <div className="ed-toolbar">
        ...
        <button onClick={() => void doDelete()} disabled={char === "h99doll"}>
          Delete
        </button>
      ...
      <div className="ed-main">
        ...
        <section className="ed-scene">
          <div className="ed-dock-title"><span className="ed-scene-tab">Scene</span><span>2D</span></div>
        ...
        <div className="ed-side">
          ...
          <div className="ed-tabs">
            {(["variants", "bones", "anim", "pixels", "spec", "catalog"] as Tool[]).map((t) => (
              ...
              >
                {t === "anim" ? "Animations" : t === "spec" ? "Template" : t === "variants" ? "Variants" : t[0]!.toUpperCase() + t.slice(1)}
              </button>
            ))}
          </div>
          <div className="ed-panel">
            ...
            {tool === "catalog" && (
              <CatalogPanel doc={doc} atlasCanvas={atlasCanvas} actions={actions} />
            )}
          </div>
        </div>
      </div>
      <footer className="ed-statusbar"><span className={dirty ? "dirty" : "ed-ready"}>●</span><span role="status">{status || (dirty ? "Unsaved changes" : "Ready")}</span><span className="spacer" /><span>{doc.spec.bones.length} bones</span><span>{doc.skeleton.slots.length} slots</span><span>Spine Character Editor</span></footer>
    </div>
  );
}
