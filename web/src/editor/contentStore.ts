...
export type { ItemDef, JobDef, QuestDef, SkillDef } from "./contentCatalogs";
...
export async function syncAllContentCatalogs(): Promise<ContentCatalogs> {
  const [entities, prefabs, tileset, items, quests, jobs, skills] = await Promise.all([
    syncCatalog(
      "entities",
      loadEntitiesLocal,
      saveEntitiesLocal,
  ...
  return { entities, prefabs, tileset, items, quests, jobs, skills };
}
