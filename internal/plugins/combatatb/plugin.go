...
func New() *Plugin {
	return &Plugin{rooms: map[string]*BattleRoom{}}
}

func (p *Plugin) ID() string         { return pluginID }
func (p *Plugin) FrontendID() string { return frontendID }

func (p *Plugin) Init(host contracts.CombatHost, config map[string]any) error {
	p.host = host
	p.speed = host.BattleSpeed()
	if v, ok := config["battle_speed"].(float64); ok && v > 0 {
		p.speed = v
	}
	return nil
}
