...
| Inspector (no selection) | `MapServerInspector` | Display name, spin-up toggle, listen addr |
...
| **Server options** | `GET` / `PUT …/server` | Read/update `*.server.json` (name, addr) + enable/disable |
